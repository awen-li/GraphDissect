...
bool input_getDirStatsAndRewind(honggfuzz_t* hfuzz) {
    ...
    PLOG_W("Analyzed '%s' directory: maxInputSz:%zu, number of usable files:%zu", hfuzz->io.inputDir,
...
bool input_init(honggfuzz_t* hfuzz) {
    hfuzz->io.fileCnt = 0U;
    PLOG_W("@@@input_init: %s ", hfuzz->io.inputDir);
...
void input_addDynamicInput(run_t* run) {
    ...
    dynfile->driver_id     = run->dynfile->driver_id;
...
bool input_prepareDynamicInput(run_t* run, bool needs_mangle) {
    ...
    run->dynfile->driver_id     = run->current->driver_id;
