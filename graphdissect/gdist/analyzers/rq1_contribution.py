...
class RQ1Contribution(Analyzer):
    ...
    def _build_driver_table(self, exe_dir: Path, g, bench_name: str, exe_name: str) -> pd.DataFrame:
        ...
        for drv_id in order:
            ...
            rows.append({
                ...
                "cfg_edge_own": int(self.block_cov.get(int(drv_id), 0)),
    ...
    def _build_summary_table(self, 
        ...
        return pd.DataFrame([{
            ...
            "sum_cfg_edge_own": int(df_drivers["cfg_edge_own"].sum()) if not df_drivers.empty else 0,
    ...
    def _compute_top_contributions(self, df_drivers: pd.DataFrame) -> pd.DataFrame:
        ...
        for metric in ["cg_node_own", "cg_edge_own", "cfg_edge_own", "bug_count"]:
