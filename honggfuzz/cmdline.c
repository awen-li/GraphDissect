...
static bool cmdlineCheckBinaryType(honggfuzz_t* hfuzz) {
    ...
    // disable automatic persitant mode
    #if 0
...
bool cmdlineParse(int argc, char* argv[], honggfuzz_t* hfuzz) {
    *hfuzz = (honggfuzz_t){
    ...
    for (;;) {
        ...
        switch (c) {
        case 'b':
            hfuzz->drv_table.dir_of_bench = optarg;
            snprintf(hfuzz->drv_table.driver_runtime_path, sizeof(hfuzz->drv_table.driver_runtime_path),
                    "%s/driver_runtimes", hfuzz->drv_table.dir_of_bench);
            char *runtime_dir = hfuzz->drv_table.driver_runtime_path;
            struct stat st = {0};
            if (stat(runtime_dir, &st) == -1) {
                if (mkdir(runtime_dir, 0755) == -1) {
                    PLOG_E("Failed to create driver_runtime directory: %s", runtime_dir);
                    return false;
                }
            }
            break;
        case 'X':
            hfuzz->drv_table.session_path = (const char*)optarg;
            snprintf(hfuzz->drv_table.active_drv_path, sizeof(hfuzz->drv_table.active_drv_path),
                    "%s/active_driver.drv", hfuzz->drv_table.session_path);
            break;
        case '!':
        ...
        case 0x600:
            ...
            break;
        case 'o':
            hfuzz->io.outputDir = strdup(optarg);
            break;
        case 0x602:
    ...
    /* load active driver */
    if (drive_loadDriver(&hfuzz->drv_table)) {
        fuzz_driver_t* driver = drive_getInitDriver(&hfuzz->drv_table);
        assert (driver != NULL);
    ...
    if (!files_exists(hfuzz->exe.cmdline[0])) {
        ...
        if (hfuzz->drv_table.session_path == NULL)
            return false;
    }
