...
bool arch_launchChild(run_t* run) {
    ...
    if (!files_writeStrToFile("/proc/self/oom_score_adj", "+500", O_WRONLY)) {
        PLOG_W("Couldn't increase our oom_score");
    }
...
static inline const char* get_Exe (honggfuzz_t* hfuzz) {
    const char *exe = hfuzz->exe.cmdline[0];
    if (hfuzz->drv_table.active_driver != NULL) {
        exe = hfuzz->drv_table.active_driver->drv_prof.driver;
    }
...
bool arch_archInit(honggfuzz_t* hfuzz) {
    ...
    const char* exe = get_Exe (hfuzz);
    if (access(exe, X_OK) == -1) {
        PLOG_E("File '%s' doesn't seem to be executable", exe);
        return false;
    }
    if ((hfuzz->arch_linux.exeFd =
                TEMP_FAILURE_RETRY(open(exe, O_RDONLY | O_CLOEXEC))) == -1) {
        PLOG_E("Cannot open the executable binary: %s)", exe);
        return false;
    }
