...
class RQ5Present(Present):
    """
    RQ5 (final-only): Region-level structural imbalance under multi-driver fuzzing.

    Input (per-executable tables emitted by RQ5RegionImbalance analyzer):
      - tables/rq5_region_imbalance__summary.csv
      - tables/rq5_region_imbalance__regions.csv
      - tables/rq5_region_imbalance__driver_region.csv
      - tables/rq5_region_imbalance__hubs.csv

    Output (paper-ready):
      1) rq5_present__imbalance_summary.csv
      2) rq5_present__top_cold_regions.csv
      3) rq5_present__cold_region_driver_alignment.csv
      4) rq5_fig__rc_union_boxplots.pdf
      5) rq5_fig__coldfrac_barh.pdf
    """

    name = "rq5"
    required_files = (
        "rq5_region_imbalance__summary.csv",
        "rq5_region_imbalance__regions.csv",
        "rq5_region_imbalance__driver_region.csv",
        "rq5_region_imbalance__hubs.csv",
    )
    ...
    def run(self) -> None:
        ...
        df_sum = self._normalize_frames(self._load_concat("rq5_region_imbalance__summary.csv"))
        df_reg = self._normalize_frames(self._load_concat("rq5_region_imbalance__regions.csv"))
        df_dr  = self._normalize_frames(self._load_concat("rq5_region_imbalance__driver_region.csv"))
        df_hub = self._normalize_frames(self._load_concat("rq5_region_imbalance__hubs.csv"))
        ...
        if not df_sum.empty:
            ...
            if miss:
                raise KeyError(f"rq5_region_imbalance__summary.csv missing columns: {miss}")

        if not df_reg.empty:
            need = {"bench", "exe", "region", "region_size", "rc_union"}
            miss = sorted(need - set(df_reg.columns))
            if miss:
                raise KeyError(f"rq5_region_imbalance__regions.csv missing columns: {miss}")

        if not df_dr.empty:
            need = {"bench", "exe", "driver", "region", "rc_driver"}
            miss = sorted(need - set(df_dr.columns))
            if miss:
                raise KeyError(f"rq5_region_imbalance__driver_region.csv missing columns: {miss}")

        pair_order = self._build_bench_order()

        # Paper table 1: per-exe imbalance summary
        tab_summary = self.build_rq5_imbalance_summary_table(df_sum, cfg=cfg)
        if not tab_summary.empty:
            tab_summary = self._apply_pair_order(tab_summary, pair_order=pair_order)
        tab_summary.to_csv(out_dir / "rq5_present__imbalance_summary.csv", index=False)

        # Paper table 2: top cold regions per exe
        tab_cold = self.build_rq5_top_cold_regions_table(df_reg, cfg=cfg)
        if not tab_cold.empty:
            tab_cold = self._apply_pair_order(tab_cold, pair_order=pair_order, extra_sort=["rc_union", "region_size"])
        tab_cold.to_csv(out_dir / "rq5_present__top_cold_regions.csv", index=False)

        # Paper table 3: cold-region vs driver alignment + top drivers
        tab_align = self.build_rq5_cold_region_driver_alignment(df_reg, df_dr, cfg=cfg)
        if not tab_align.empty:
            tab_align = self._apply_pair_order(tab_align, pair_order=pair_order, extra_sort=["region", "rank"])
        tab_align.to_csv(out_dir / "rq5_present__cold_region_driver_alignment.csv", index=False)

        # Optional: keep suite-wide raw data for appendix/debug
        df_sum.to_csv(out_dir / "rq5_present__summary_raw.csv", index=False)
        df_reg.to_csv(out_dir / "rq5_present__regions_raw.csv", index=False)
        df_dr.to_csv(out_dir / "rq5_present__driver_region_raw.csv", index=False)
        df_hub.to_csv(out_dir / "rq5_present__hubs_raw.csv", index=False)

        # Paper appendix helper: SNDP hub stats (top degree hubs)
        tab_hubs = self.build_rq5_hubs_table(df_hub, topk=15)
        tab_hubs.to_csv(out_dir / "rq5_present__top_hubs.csv", index=False)
    ...
    def post_run(self) -> None:
        ...
        self.plot_rq5_rc_union_boxplots(df_reg, out_dir, cfg=cfg)
        self.plot_rq5_coldfrac_barh(df_sum, out_dir, cfg=cfg)

        # merged two-panel figure
        self.plot_rq5_two_panel_imbalance(df_sum, df_reg, out_dir, cfg=cfg)
    ...
    # TABLES
    # -----------------------------
    def build_rq5_imbalance_summary_table(self, df_sum: pd.DataFrame, cfg: _RQ5Cfg) -> pd.DataFrame:
    ...
    def build_rq5_top_cold_regions_table(self, df_reg: pd.DataFrame, cfg: _RQ5Cfg) -> pd.DataFrame:
    ...
    def build_rq5_cold_region_driver_alignment(
        self,
        df_reg: pd.DataFrame,
        df_dr: pd.DataFrame,
        cfg: _RQ5Cfg,
    ...
    def build_rq5_hubs_table(self, df_hub: pd.DataFrame, topk: int = 15) -> pd.DataFrame:
    ...
    # FIGURES
    # -----------------------------
    def plot_rq5_rc_union_boxplots(self, df_reg: pd.DataFrame, out_dir: Path, cfg: _RQ5Cfg) -> Path:
        """
        Boxplot of RC_union distribution across regions per executable.
        Regions with size < MIN_REGION_SIZE are excluded.
        """
        out_dir.mkdir(parents=True, exist_ok=True)
        out_path = out_dir / "rq5_fig__rc_union_boxplots.pdf"

        if df_reg is None or df_reg.empty:
            fig, ax = plt.subplots(1, 1, figsize=(7.0, 3.0))
            ax.text(0.5, 0.5, "RQ5: no data", ha="center", va="center")
            ax.axis("off")
            fig.tight_layout()
        ...
        if tmp.empty:
            fig, ax = plt.subplots(1, 1, figsize=(7.0, 3.0))
            ax.text(0.5, 0.5, f"RQ5: no regions with |R| ≥ {cfg.min_region_size}", ha="center", va="center")
            ax.axis("off")
            fig.tight_layout()
            fig.savefig(out_path, dpi=300)
            plt.close(fig)
        ...
        ax.set_title(f"RQ5: Region coverage distribution (|R|≥{cfg.min_region_size})", fontsize=10)
    ...
    def plot_rq5_coldfrac_barh(self, df_sum: pd.DataFrame, out_dir: Path, cfg: _RQ5Cfg) -> Path:
        """
        Horizontal bar chart of ColdFrac per executable.
        """
        out_dir.mkdir(parents=True, exist_ok=True)
        out_path = out_dir / "rq5_fig__coldfrac_barh.pdf"
        ...
        if tmp.empty:
            fig, ax = plt.subplots(1, 1, figsize=(7.0, 3.0))
            ax.text(0.5, 0.5, "RQ5: no valid cold_frac values", ha="center", va="center")
            ax.axis("off")
            fig.tight_layout()
            fig.savefig(out_path, dpi=300)
            plt.close(fig)
        ...
        ax.set_title("RQ5: Fraction of under-explored structural regions", fontsize=10)
        ax.tick_params(axis="y", labelsize=10)
        ax.tick_params(axis="x", labelsize=10)
    ...
    def plot_rq5_two_panel_imbalance(
        self,
        df_sum: pd.DataFrame,
        df_reg: pd.DataFrame,
        out_dir: Path,
        ...
        Output:
          rq5_fig__two_panel_imbalance.pdf
        """
        out_dir.mkdir(parents=True, exist_ok=True)
        out_path = out_dir / "rq5_fig__two_panel_imbalance.pdf"
        ...
        if df_reg is None or df_reg.empty or "rc_union" not in df_reg.columns:
            fig, ax = plt.subplots(1, 1, figsize=(7.0, 3.0))
            ax.text(0.5, 0.5, "RQ5: regions missing", ha="center", va="center")
            ax.axis("off")
            fig.tight_layout()
            fig.savefig(out_path, dpi=300)
            plt.close(fig)
        ...
        if len(keep_labels) == 0:
            ...
            ax.text(
                0.5,
                0.5,
                f"RQ5: no regions with |R|≥{cfg.min_region_size} for any executable",
                ha="center",
                va="center",
            )
            ax.axis("off")
